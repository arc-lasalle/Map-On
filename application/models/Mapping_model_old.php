...
class Mapping_model extends CI_Model 
{
	...
	function generateSQL($tableRcolumn, $mappedclass_id, $datasource_id)
	{	
		$sql = "";
...
//				$('#input_select_path').value = pathHtml;
	...
	*/
	...
	//Intern function
	function generateURI($input_class, $input_table, $basic_uri, $ontology_id)
	{
		...
		if(count($arr2) == 2) {
			if(count($arr) == 2) {
				return $basic_uri.strtolower($arr2[1]).'/{'.strtolower($arr[0].$arr[1]).'}';
			} else {
