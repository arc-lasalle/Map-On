    <script type="text/javascript">
        function main () {
			...
			foreach($nodes as $row) {
				...
				echo "graph.addNode('".$row."', {text: '".$row."', type:1, description:''});\n";
				echo "graph.addLink('".$root."', '".$row."', { connectionStrength: 0.65, type: 1});\n\n";
		...
		?>
			...
			selectPath = function (nodeid) {
				...
				pathHtml = recursivePathHTMLDFS (nodeid, visited, graph, 1);
			...
			recursivePathHTMLDFS = function (nodeid, visited, graph, backlinktype) {
				visited[nodeid] = 1;
				var ret = "";
				...
				graph.forEachLinkedNode(nodeid, function(nodeTo, link){
					if(nodeTo.id in visited && ret == "") {
						...
						retRec = recursivePathHTMLDFS(nodeTo.id, visited, graph, link.data.type);
...
//				console.log(pos);
	...
	<div class="ui segment ">
		<span>Path: <span id="select_path">
			<div class="ui small orange circular image label" style="cursor:pointer;" ><?php echo $root;?></div>
