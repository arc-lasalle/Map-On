    <script type="text/javascript">
        ...
        function main () {
					...
					foreach($columns[$row->id] as $col) {
						...
						if($col->foreignkey != "" && $col->foreigntable != "") {
							if(array_key_exists(strtolower($col->foreigntable), $mapTables)) {
								echo "graph.addLink('".strtolower($col->foreigntable."_".$col->foreignkey)."', '".strtolower($row->name."_".$col->name)."', { connectionStrength: 0.7, type: 5});\n\n";
							}
						}
			...
			echo "node = graph.addNode('".$mapClass->class."', {text: '".$mapClass->class."', type:1, isPinned: true, description:'".str_replace("\n", ' ', $mappedclassDescription)."'});\n";
			...
			foreach($datapropertiesList as $row) {			
				if($dataproperty_id == $row["id"]) {
					echo "node = graph.addNode(':newdataprop:', {text: '".$row["dp"]."', type:4, isPinned: true});\n";
					
					if(array_key_exists(':newdataprop:', $layout)) {
						$posx = $layout[':newdataprop:']["layoutX"];
						$posy = $layout[':newdataprop:']["layoutY"];
						echo "layout.setNodePosition(node, ".$posx.", ".$posy.");";
					}
					...
					if(array_key_exists($row["dp"], $layout)) {
						$posx = $layout[$row["dp"]]["layoutX"];
						$posy = $layout[$row["dp"]]["layoutY"];
						echo "layout.setNodePosition(node, ".$posx.", ".$posy.");";
					}
			...
			//Object & data & mappings properties		
				...
				foreach($dataproperties[$mapClass->id] as $objp) {
					if($dataproperty_id == $objp->id) {
						echo "graph.addLink('".$mapClass->class."', ':newdataprop:', { connectionStrength: 0.7, type: 4});\n\n";
					} else {
						echo "graph.addLink('".$mapClass->class."', '".$objp->dataproperty."', { connectionStrength: 0.7, type: 4});\n\n";
					}
				}

				//Mappings class
				if($mappings[$mapClass->id] != "") {
					echo "graph.addLink('".$mapClass->class."', '".strtolower($mappings[$mapClass->id])."', { connectionStrength: 0.1 , type: 3});\n\n";
				}
			...
			foreach($mappingsDP as $row) {
				if($dataproperty_id == $objp->id) {
					echo "graph.addLink(':newdataprop:', '".strtolower($row["table"])."', { connectionStrength: 0.1, type: 3});\n\n";	
				} else {
					echo "graph.addLink('".$row["dp"]."', '".strtolower($row["table"])."', { connectionStrength: 0.1, type: 3});\n\n";
				}
			}
			?>
			...
			$('#centerForm').submit(function(e) {
				...
				if(action === "edit") {
					...
					graph.forEachLinkedNode(':newdataprop:', function(linkedNode, link){
						graph.removeLink(link); 
					});
					
					var position = layout.getNodePosition(':newdataprop:');
					graph.removeNode(':newdataprop:');
					
					
					graph.addNode(':newdataprop:', {text: document.getElementById('nodeid').value, type:4, isPinned: true});	
					graph.addLink(document.getElementById('mapclass').value, ':newdataprop:', { connectionStrength: 0.7, type: 4});
					graph.addLink(document.getElementById('table').value.toLowerCase(), ':newdataprop:', { connectionStrength: 0.1, type: 3});
					layout.setNodePosition(node, position.x, position.y);
					/*
					nodeUI = graphics.getNodeUI(':newdataprop:');
					...
					*/
					...
					graph.addLink(table, ':newdataprop:', { connectionStrength: 0.1, type: 3});
				}
			});
			
		//This is because 
		<?php if($dataproperty_id == 0) { ?>
			//var node = graph.addNode(':newdataprop:', {text: document.getElementById('nodeid').value, type:4});	
			
			<?php 
				if(array_key_exists(':newdataprop:', $layout)) {
					echo "	var node = graph.addNode(':newdataprop:', {text: document.getElementById('nodeid').value, type:4, isPinned:true});	";
					$posx = $layout[':newdataprop:']["layoutX"];
					$posy = $layout[':newdataprop:']["layoutY"];
					echo "layout.setNodePosition(node, ".$posx.", ".$posy.");";
				} else {
					echo "	var node = graph.addNode(':newdataprop:', {text: document.getElementById('nodeid').value, type:4});	";
				}
			?>
		<?php } ?>
